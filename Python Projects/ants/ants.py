...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        count = 0
        while cursor != None:
            if len(cursor.bees) != 0 and cursor is not hive and count >= self.min_range and count <= self.max_range:
                return random_or_none(cursor.bees)
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.impostor:
            ...
            cursor = self.place
            if cursor.ant.container == True and cursor.ant not in self.doubled:
                cursor.ant.damage *= 2
                self.doubled.append(cursor.ant)
            cursor = cursor.exit
            while cursor != None:
                if cursor.ant != None:
                    if cursor.ant.container == True:
                        if cursor.ant not in self.doubled:
                            cursor.ant.damage *= 2
                            self.doubled.append(cursor.ant)
                        if cursor.ant.ant != None and cursor.ant.ant not in self.doubled:
                            cursor.ant.ant.damage *= 2
                            self.doubled.append(cursor.ant.ant)
                    else:
                        if cursor.ant not in self.doubled:
                            cursor.ant.damage *= 2
                            self.doubled.append(cursor.ant)
