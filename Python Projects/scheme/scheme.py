...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        return Procedure.eval_call(proc, rest, env)
        # END PROBLEM 5
...
def eval_all(expressions, env):
    ...
    if expressions == nil:
        return None
    cursor = expressions
    while cursor.second != nil:
        scheme_eval(cursor.first, env)
        cursor = cursor.second
    return scheme_eval(cursor.first, env)
    # END PROBLEM 8
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        parameters = operands.map(lambda arg: scheme_eval(arg,env))
...
def do_and_form(expressions, env):
    ...
    cursor = expressions
    while cursor.second != nil:
        if scheme_falsep(scheme_eval(cursor.first, env)):
            return False
        cursor = cursor.second
    return scheme_eval(cursor.first, env)
    # END PROBLEM 13
...
def do_or_form(expressions, env):
    ...
    cursor = expressions
    while cursor != nil:
        if scheme_truep(scheme_eval(cursor.first, env)):
            return scheme_eval(cursor.first, env)
        cursor = cursor.second
    return False
    # END PROBLEM 13
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    symbols, formals = nil, nil
    while bindings is not nil:
        check_form(bindings.first, 2, 2)
        expr = scheme_eval(bindings.first.second.first, env)
        symbols = Pair(expr, symbols)
        formals = Pair(bindings.first.first, formals)
        bindings = bindings.second
    check_formals(formals)
    return env.make_child_frame(formals, symbols)
